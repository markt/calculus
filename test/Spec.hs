...
main = defaultMain (testGroup "Library Tests" [varTest,valTest,conTest,funTest,lawTest,lawNameTest]) --test3, test4, test5, test6, test7, test8, test9, test10])
...
exampleExpr2 :: Expr
exampleExpr2 = Deriv "x" (Con "sin" [Con "^" [Var "x", Val 2]])
...
lawNameTest =
 LC.testProperty "calc test" ((calculate lawList exampleExpr2) == exampleCalc)
